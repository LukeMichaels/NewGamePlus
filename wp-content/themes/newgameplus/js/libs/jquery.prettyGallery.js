...
jQuery.fn.prettyGallery = function(settings) {
	...
	return this.each(function(){
		...
		var prettyGalleryPrevious = function(caller) {
			...
			$gallery.find('dt:lt('+(currentPage * settings.itemsPerPage)+')').each(function(i){
				$(this).animate({'left': parseFloat($(this).css('left')) + (galleryWidth + itemMargin) }, settings.animationSpeed, function(){
					animated = false;
				});
			});

			$gallery.find('dt:gt('+ ((currentPage * settings.itemsPerPage) - 1) +')').each(function(i){
		...
		var _formatGallery = function() {
			itemWidth = $gallery.find('dt:first').css('position','absolute').width();
			
			itemMargin += ($gallery.find('dt:first').css('margin-right') == 'auto') ? 0 : parseFloat($gallery.find('dt:first').css('margin-right'));
			itemMargin += ($gallery.find('dt:first').css('margin-left') == 'auto') ? 0 : parseFloat($gallery.find('dt:first').css('margin-left'));
			itemMargin += ($gallery.find('dt:first').css('padding-right') == 'auto') ? 0 : parseFloat($gallery.find('dt:first').css('padding-right'));
			itemMargin += ($gallery.find('dt:first').css('padding-left') == 'auto') ? 0 : parseFloat($gallery.find('dt:first').css('padding-left'));

			var itemSpacingTopBottom = 0;
				itemSpacingTopBottom += ($gallery.find('dt:first').css('margin-top') == 'auto') ? 0 : parseFloat($gallery.find('dt:first').css('margin-top'));
				itemSpacingTopBottom += ($gallery.find('dt:first').css('margin-bottom') == 'auto') ? 0 : parseFloat($gallery.find('dt:first').css('margin-bottom'));
				itemSpacingTopBottom += ($gallery.find('dt:first').css('padding-top') == 'auto') ? 0 : parseFloat($gallery.find('dt:first').css('padding-top'));
				itemSpacingTopBottom += ($gallery.find('dt:first').css('padding-bottom') == 'auto') ? 0 : parseFloat($gallery.find('dt:first').css('padding-bottom'));

			var itemHeight = $gallery.find('dt:first').height() + itemSpacingTopBottom;
			var marginRight = ($gallery.find('dt:first').css('margin-right') == 'auto') ? 0 : parseFloat($gallery.find('dt:first').css('margin-right'));
			...
			$gallery.find('dt').each(function(i){
				$(this).css({
					'position':'absolute',
					'top':0,
		...
		var _applyNav = function() {
			var template = '';
				...
				template += '<li class="gallery-pagination"><span class="current">1</span>'+settings.of_label+'<span class="total">1</span></li>';
		...
		if($(this).find('dt').size() > settings.itemsPerPage) {		
			// Set the number of pages
			pageCount = Math.ceil($(this).find('dt').size() / settings.itemsPerPage);
